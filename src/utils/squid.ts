...
export const getRoute = async () => {
  ...
  const fromChain = 250;
  const fromToken = '0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83';
  const fromAmount = ethers.parseUnits("1.4", 18).toString();
  ...
  const params = {
    ...
    enableForecall,
    quoteOnly,
    customContractCalls: [generateCustomContractCall("transfer", [stratAddress, "0"], toToken)]
  };
...
export const encodeData =
  (functionName: string, args: unknown[], abi = erc20Abi) => {
    const iface = new ethers.Interface(abi);
    return iface.encodeFunctionData(functionName, args);
  }

export const prepareWriteTx =
  async (args: unknown[], to: string, functionName: string, abi = erc20Abi) => {
...
export const approve = async (routerAddress: string, amountInWei: string, tokenAddress: string) => {
  return await writeContract(
    await prepareWriteTx([routerAddress, amountInWei], tokenAddress, 'approve'));
}

export const generateCustomContractCall =
  (functionName: string, args: unknown[], toToken: string, abi = erc20Abi): ICustomContractCall => {
    return {
