...
const SwapBlock = ({
  ...
  const balance = useMemo(() => {
    ...
    return amountToEth(BigInt(balance?.amount ?? 0), token.decimals);
  }, [token]);
  ...
  const tokenPrice = useMemo(() => {
    ...
    if ('sharePrice' in token) {
      ...
      price = (Number(tokenPrices[asset.coinGeckoId]?.usd) * sharePrice)
        / weiPerUnit;
    }
    ...
    <div className="flex flex-col my-3">
      ...
      <div className="flex flex-col md:flex-row p-3 border border-solid border-gray-500 rounded-xl bg-dark-600">
        ...
        <div className="flex flex-col ms-auto my-auto text-right">
          <div
            className={
              clsx("text-sm flex flex-row align-middle ms-auto bg-gray-200 rounded px-1", {
